...
Public Class Timer
    ...
    Public Sub New(Initlog As Log)
        log = Initlog
    End Sub
    Friend Function isTimeToRun(task As SGConsoleApp.Settings.Task, ByRef fileList As List(Of String)) As Boolean
        ...
        ElseIf LCase(task.Schedule.Type) = "file" Then
            If task.Schedule.FileSetting.SpoolFolder > "" And task.Schedule.FileSetting.ProcessFolder > "" Then
                ...
                Dim fileNames As String() = System.IO.Directory.GetFiles(task.Schedule.FileSetting.SpoolFolder)
        ...
        Return isTime
    End Function
    Friend Function createDirecotries(task As SGConsoleApp.Settings.Task) As Boolean
    ...
    Public Sub SetNextRun(Task As Settings.Task)
        Dim logErrorInfo As String = "init"
        Try
            ...
            End If
            ' Set Hour and Minute:
            logErrorInfo = "Hour and Minute handling"
            ...
            log.Logita("TIME SET: Task " & Task.Name & ", next run=" & Task.NextRun.ToString())
        Catch ex As Exception
            Throw New Exception("Error in Timer at " & logErrorInfo & ": " & ex.Message)
        End Try
    End Sub
